...
1. Fork the project by navigating to the main [repository](https://github.com/carbon-design-system/carbon-polyfill) and clicking the **Fork** button on the top-right corner.

2. Navigate to your forked repository and copy the **SSH url**. Clone your fork by running the following in your terminal:

	```
	$ git clone git@github.com:{ YOUR_USERNAME }/carbon-polyfill.git
	$ cd carbon-polyfill
	```

	See [GitHub docs](https://help.github.com/articles/fork-a-repo/) for more details on forking a repository.
	
3. Once cloned, you will see `origin` as your default remote, pointing to your personal forked repository. Add a remote named `upstream` pointing to the main `carbon-polyfill`:

	```
	$ git remote add upstream git@github.com:carbon-design-system/carbon-polyfill.git
	$ git remote -v
	```
...
7. In Github, navigate to [carbon-design-system/carbon-polyfill](https://github.com/carbon-design-system/carbon-polyfill) and click the button that reads "Compare & pull request".
...
We use commit message guidelines based on the [Angular Commit Conventions](https://github.com/angular/angular.js/blob/master/CONTRIBUTING.md#commit). After the commit message has been submitted, it is checked by [`husky`](https://www.npmjs.com/package/husky) and [`commitizen`](https://www.npmjs.com/package/commitizen) to ensure it is syntactically correct.
